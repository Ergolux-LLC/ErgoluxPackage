...
class ConfigDict(TypedDict, total=False):
    ...
    t = {{ table_name|capitalize }}(user=my_user)
    print(t)
    #> user=User(hobbies=['reading'])

    my_user.hobbies = [1]  # (2)!
    t = {{ table_name|capitalize }}(user=my_user)  # (3)!
    print(t)
    #> user=User(hobbies=[1])

    my_sub_user = SubUser(hobbies=['scuba diving'], sins=['lying'])
    t = {{ table_name|capitalize }}(user=my_sub_user)
    print(t)
    ...
    my_user.hobbies = [1]
    try:
        t = {{ table_name|capitalize }}(user=my_user)  # (2)!
    except ValidationError as e:
        print(e)
        '''
        1 validation error for {{ table_name|capitalize }}
        user.hobbies.0
